...
func isOverlyBroad(domain string) bool {
	...
	prefix := strings.TrimSuffix(trailingDomain, "."+tld)
	if prefix == trailingDomain || prefix == "" {
		return true
	}

	// Check if prefix consists only of wildcards (*, %) and dots
	// e.g., "%" from "%.com" or "%.%" from "%.%.co.uk"
	cleaned := strings.ReplaceAll(prefix, "%", "")
