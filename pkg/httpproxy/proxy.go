...
// that would allow proxying to essentially any domain.
func isOverlyBroad(domain string) bool {
	if domain == "%" || domain == "*" {
		return true
	}

	trailingDomain := domain
	if strings.HasPrefix(trailingDomain, "*.") {
		trailingDomain = trailingDomain[2:]
	} else if strings.HasPrefix(trailingDomain, "*") {
		trailingDomain = trailingDomain[1:]
	}

	// Get the public suffix (TLD)
	tld, _ := publicsuffix.PublicSuffix(trailingDomain)
	if tld == "" {
		return false
	}

	leadingURL := strings.TrimSuffix(trailingDomain, "."+tld)
	if leadingURL == trailingDomain || leadingURL == "" {
		return true
	}

	// Check if leadingURL consists only of wildcards (*, %) and dots
	// e.g., "%" from "%.com" or "%.%" from "%.%.co.uk"
	cleaned := strings.ReplaceAll(leadingURL, "%", "")
