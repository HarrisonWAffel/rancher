...
func areSecretsSame(s1, s2 *corev1.Secret) bool {
	s1Anno := s1.Annotations
	if s1Anno == nil {
		s1Anno = map[string]string{}
	}
	s2Anno := s2.Annotations
	if s2Anno == nil {
		s2Anno = map[string]string{}
	}
	return reflect.DeepEqual(s1.Data, s2.Data) &&
		s1Anno[ProjectScopedSecretLabel] == s2Anno[ProjectScopedSecretLabel] &&
		s1Anno[pssCopyAnnotation] == s2Anno[pssCopyAnnotation]
}
...
func secretIgnoresNamespace(annos map[string]string, namespace string) bool {
	v, ok := annos[PSSIgnoreNamespacesAnnotations]
	if !ok {
		return false
	}
