...
func (n *namespaceHandler) OnChange(_ string, namespace *corev1.Namespace /*This is the backing namespace of a specific project, of which a secret is associated with*/) (*corev1.Namespace, error) {
...
func (n *namespaceHandler) removeUndesiredProjectScopedSecrets(namespace *corev1.Namespace, upstreamDesiredSecrets sets.Set[types.NamespacedName]) error {
	...
	downstreamGlobalPullSecrets, err := n.secretClient.List(namespace.Name, metav1.ListOptions{
		LabelSelector: "management.cattle.io/rancher-managed-pull-secret",
	})
	if err != nil {
		return err
	}
...
func (n *namespaceHandler) secretEnqueueNamespace(_, _ string, obj runtime.Object /*This is actually a secret*/) ([]relatedresource.Key, error) {
	if obj == nil {
		return nil, nil
	}
...
func (n *namespaceHandler) getNamespacesFromGlobalPullSecret(secret *corev1.Secret) ([]*corev1.Namespace, error) {
	...
	_, isDefaultRegistrySecret := secret.Labels["management.cattle.io/rancher-default-registry-pull-secret"]
...
// getGlobalPullSecrets lists secrets in the local clusters cattle-system namespace which have the 'management.cattle.io/rancher-default-registry-pull-secret'
// label. The returned secrets are then compared against
func (n *namespaceHandler) getGlobalPullSecrets() ([]*corev1.Secret, error) {
	// Get the source secrets
	globalPullSecrets, err := n.managementSecretCache.List(namespaces.System, labels.SelectorFromSet(map[string]string{
		"management.cattle.io/rancher-default-registry-pull-secret": "true",
	}))
	if err != nil {
		return nil, err
	}
	...
	specifiedSecretsSet := sets.New[string]().Insert(registry.PullSecretNamesAsSlice()...)

	// of all the globally defined pull secrets, isolate the ones that exist and are
	// supposed to be used. This will be used later to identify secrets which should no longer
	// exist downstream. Because PSS's typically copy the source secret verbatim, we need to massage
	// some of the labels to properly denote that they are copies. This will be used later to ensure
	// proper cleanup of these resources in imported clusters.
	var downstreamPullSecrets []*corev1.Secret
	for _, secret := range globalPullSecrets {
		if !specifiedSecretsSet.Has(secret.Name) {
			continue
		}
		...
		delete(secretCopy.Labels, "management.cattle.io/rancher-default-registry-pull-secret")
		secretCopy.Labels["management.cattle.io/rancher-managed-pull-secret"] = "true"
		if secret.Type != corev1.SecretTypeDockerConfigJson {
			secret.Type = corev1.SecretTypeDockerConfigJson
		}
		downstreamPullSecrets = append(downstreamPullSecrets, secretCopy)
	}

	return downstreamPullSecrets, nil
}
