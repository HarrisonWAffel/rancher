...
func ConvertToDockerConfigJson(secretType kcorev1.SecretType, registryHost string, data map[string][]byte) ([]byte, error) {
	switch secretType {
	case "rke.cattle.io/auth-config":
		auth, ok := data["auth"]
		if !ok {
			return nil, fmt.Errorf("auth config not found in secret")
		}
	...
	case kcorev1.SecretTypeDockerConfigJson:
		return data[kcorev1.DockerConfigJsonKey], nil
	default:
...
func UnwrapDockerConfigJson(registryHostname string, configJson map[string][]byte) (string, string, string, error) {
