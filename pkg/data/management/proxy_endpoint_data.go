...
func (h *handler) manageDefaultProxyEndpoints(_ string, setting *v3.Setting) (*v3.Setting, error) {
	if setting.Name != "disable-default-proxy-endpoint" {
		return setting, nil
	}
...
func AddProxyEndpointData(disabledEndpointsSetting string, clients *wrangler.Context) error {
	...
	disableAll := slices.Contains(disabledEndpoints, "all")
	...
	for _, endpoint := range builtInEndpoints {
		err := createOrDisableEndpoint(endpoint.endpoint, slices.Contains(disabledEndpoints, endpoint.name) || disableAll, clients)
		if err != nil {
			return err
		}
	}
